     ...
     public static int numCombinationForFinalScore(int finalScore, List<Integer> individualPlayScore){
        ...
        for(int i=0;i<individualPlayScore.size();i++){
            ...
            for(int j=i;j<=finalScore;++j){
                ...
                numCombinationForScore[i][j] = withThisPlay+withoutThisPlay;
            }
        }
        return numCombinationForScore[individualPlayScore.size()-1][finalScore];
    }
//Time  Complexity of the above the program is O(sn) and the space complexity is O(sn)
